import type {ConnectorOutput, ServiceAccountContext} from '../connectors/types.js';
...
export class PartageonsLeauClient {
   ...
   *
   */
	async getContextForServiceAccount(serviceAccount: string): Promise<ServiceAccountContext | undefined> {
   ...
   */
	async ingest(output: ConnectorOutput): Promise<void> {
		...
		console.log(`[PartageonsLeauClient] Ingesting ${metricCount} metrics (${valueCount} values) for service account: ${output.serviceAccount} and point: ${output.pointId}`);
	}
}
